...
# A key for the orbit dictionary: the rows of mat as one vector over F.
BindGlobal( "MatrixKey", function( mat, F )
    return ImmutableVector( F, Concatenation( mat ) );
end );

# A dictionary for keys like key.  Its hash function is chosen for the
# row space over F: chosen from key alone it would be one for the field
# generated by the entries of key, and fail on other keys.
BindGlobal( "MatrixKeyDictionary", function( key, F )
    return NewDictionary( key, true, F^Length(key) );
end );
